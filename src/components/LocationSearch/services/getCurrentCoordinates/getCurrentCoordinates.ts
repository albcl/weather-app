...
} from "src/errors/createErrorFactory";
import { GEOLOCATION_ERRORS } from "src/errors/errors";
...
export async function getCurrentCoordinates(): Promise<any> {
  return await new Promise((resolve, reject) => {
    if (!(navigator && "geolocation" in navigator))
      reject(new NoNavigatorAvailable(""));
